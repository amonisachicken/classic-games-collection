...
impl<'a> Engine<'a> {
    ...
    fn draw_pause(&mut self) {
        ...
        let l1 = lang::ui().resume;
        let l2 = lang::ui().quit_to_menu;
        let l3 = lang::ui().tip_ctrlc;
...
fn draw_summary(eng: &mut Engine, id: GameId, outcome: GameOutcome) {
    ...
    row += 1;
    let title = lang::fmt(lang::ui().leaderboard_fmt, &[&id.display_name()]);
    ...
    if id == GameId::Gomoku {
        ...
        let hdr = lang::ui().lb_header_gomoku;
        ...
        let hdr = lang::ui().lb_header_score;
        c.put_str(cx - str_width(hdr) / 2, row, hdr, col::GRAY, col::BLACK);
        row += 1;
...
fn draw_username(eng: &mut Engine, buf: &str, msg: &str) {
    ...
    c.fill_rect(x, y, w, h, ' ', col::BLACK, col::DARK_BLUE);
    c.border(x, y, w, h, col::CYAN);
    let t = lang::ui().set_username;
    c.put_str(x + (w - str_width(t)) / 2, y + 1, t, col::YELLOW, col::DARK_BLUE);
    c.put_str(x + 2, y + 3, lang::ui().username, col::WHITE, col::DARK_BLUE);
